...
Slack에서 `@팡이`에게 분석을 요청하면, 팡이는 허용된 코드 저장소를 안전한 read-only worktree로 분리한 뒤 Codex CLI로 코드를 읽고 결과를 Slack thread에 답합니다.
...
flowchart TD
    ...
    C --> D["SQLite에 AgentJob 저장"]
    D --> E["Background worker 실행"]
    E --> F["허용된 source repo 확인"]
    F --> G["Read-only git worktree 생성"]
    G --> H["Codex exec --sandbox read-only 실행"]
    H --> I["stdout, stderr, exit code, timeout 저장"]
    I --> J["Slack thread에 결과 응답"]
```

현재 단계에서 팡이는 코드를 수정하지 않습니다. 먼저 코드를 읽고, 확인한 사실과 근거를 정리하는 역할에 집중합니다.
