...
def test_plan_is_read_only_and_apply_is_idempotent(tmp_path: Path) -> None:
    ...
    assert [migration.version for migration in plan.pending] == [
        1,
        2,
        3,
        4,
...
def test_packaged_connection_registry_migration_upgrades_v8_with_verified_backup(
    tmp_path: Path,
) -> None:
    paths, config = _initialized_runtime(tmp_path)
    packaged = PackageMigrationRegistry().load()
    previous = StaticMigrationRegistry(*packaged[:8])
    asyncio.run(
        SqliteMigrationAdmin(
            paths,
            config.storage,
    ...
    result = asyncio.run(
        SqliteMigrationAdmin(
            paths,
            config.storage,
            registry=StaticMigrationRegistry(*packaged[:9]),
        ).apply()
    )

    assert result.current_version == 9
    assert [migration.name for migration in result.applied] == ["connection_registry"]
    assert result.backup_file is not None
    with sqlite3.connect(paths.database_file) as connection:
        tables = {
            row[0]
    ...
    assert {"connections", "connection_tools"} <= tables
    with sqlite3.connect(result.backup_file) as backup:
        assert backup.execute("PRAGMA user_version").fetchone() == (8,)
        assert (
            backup.execute(
                "SELECT name FROM sqlite_master WHERE name = 'connections'"
            ).fetchone()
            is None
        )


def test_packaged_tool_policy_budget_migration_upgrades_v9_with_verified_backup(
    tmp_path: Path,
) -> None:
    paths, config = _initialized_runtime(tmp_path)
    packaged = PackageMigrationRegistry().load()
    previous = StaticMigrationRegistry(*packaged[:9])
    asyncio.run(
        SqliteMigrationAdmin(
            paths,
            config.storage,
    ...
    assert {"tool_policies", "tool_call_budgets"} <= tables
    with sqlite3.connect(result.backup_file) as backup:
...
def test_packaged_tool_approval_migration_upgrades_v10_with_verified_backup(
    ...
    result = asyncio.run(
        SqliteMigrationAdmin(
            paths,
            config.storage,
            registry=StaticMigrationRegistry(*packaged[:11]),
        ).apply()
    )
...
def test_packaged_tool_invocation_migration_upgrades_v11_with_verified_backup(
    tmp_path: Path,
) -> None:
    paths, config = _initialized_runtime(tmp_path)
    packaged = PackageMigrationRegistry().load()
    previous = StaticMigrationRegistry(*packaged[:11])
    asyncio.run(
        SqliteMigrationAdmin(
            paths,
            config.storage,
    ...
    with sqlite3.connect(paths.database_file) as connection:
        ...
        assert (
            connection.execute(
                "SELECT name FROM sqlite_master WHERE name = 'tool_invocations'"
            ).fetchone()
            is not None
        )
    with sqlite3.connect(result.backup_file) as backup:
        assert backup.execute("PRAGMA user_version").fetchone() == (11,)
        assert (
            backup.execute(
                "SELECT name FROM sqlite_master WHERE name = 'tool_invocations'"
            ).fetchone()
