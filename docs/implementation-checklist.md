...
- [x] 1차 MVP 목표를 아래 한 문장으로 확정한다.
  ...
  - 권장 문장: "Slack에서 팡이에게 분석 요청을 보내면, 서버가 Slack thread와 1:1로 대응되는 Codex session과 thread workspace에서 Codex read-only 분석을 실행하고 결과를 Slack thread에 반환한다."
...
- [x] 중복 event id 방지 필드를 추가한다.
  - 완료 기준: Slack retry가 같은 job과 user message를 재사용한다.
...
- [ ] worktree root 경로를 설정한다.
  - 완료 기준: thread workspace와 repo checkout이 생성될 수 있다.
...
- [x] Slack thread별 active Codex session 유지
- [x] Slack thread별 1시간 idle session archive와 workspace cleanup
- [ ] PR 요약 기능
