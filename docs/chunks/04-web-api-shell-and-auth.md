...
- `/`, `/assets/*`, `/api/v1/*`와 Health의 공통 Server 구조를 만든다.
- Local Bootstrap과 영속 Session의 인증 경계를 정의한다.
- 상태 변경 API의 인증, CSRF와 Error Envelope를 공통화한다.
...
- 공통 Error Envelope와 역할 기반 API Dependency
- OpenAPI 생성 Type과 Frontend API Client 기반
- Live/Ready Endpoint와 전체 Admin Layout
...
- Slack Event Adapter
- Slack OIDC와 Reverse Proxy OIDC Adapter
- 실제 기능 API의 Idempotency, Cursor, Resource Owner와 SSE
...
- 현재 Dashboard 인증은 Local Bootstrap으로 만든 영속 Session을 사용한다. Slack OIDC는 WBS-16, Reverse Proxy OIDC는 WBS-18에서 같은 Identity·Session 경계에 연결한다.
- 첫 Admin 생성 Transaction이 Bootstrap Token을 즉시 폐기한다.
- WBS-03 Unit of Work 위에서 `users`, `auth_identities`, `auth_sessions`, `bootstrap_grants`의 Migration, 제약과 Repository를 이 WBS가 소유한다.
...
- Member, Skill Author, Admin, System 역할을 API Dependency에서 검사한다. Resource Owner 조건은 실제 기능 Use Case를 소유한 후속 WBS가 재검사한다.
- 모든 오류는 Stable Code, 안전한 Message, Request ID와 제한된 Details를 가진 Envelope로 변환한다.
- React Router와 OpenAPI 생성 Type을 기본으로 하고 Build Gate에서 Schema Drift를 검사한다. TanStack Query는 실제 Server State 화면이 생기는 후속 WBS에서 도입한다.
...
3. **공통 API 보안 계약**: Request ID, Error Envelope, 역할과 CSRF를 만든다.
4. **Admin UI 계약**: 전체 Sidebar/Layout, OpenAPI 생성 Type, API Client와 Schema Drift Gate를 완성한다.
...
- 이 단계에서는 실제 기능 API와 함께 검증해야 하는 Idempotency·Cursor·Resource Owner, 지원 Header 결정이 필요한 Slack/Reverse Proxy OIDC, 전체 Admin Layout과 SSE를 구현하지 않았다.

## 5차 구현 결과

- 설계서의 전체 일반·관리자 Navigation을 하나의 Metadata로 정의하고 `admin` 역할에서만 관리자 전용 Group을 표시한다.
- 인증 화면, Session Gate, Overview와 Admin Layout을 기능·표현 책임별로 분리했다.
- Desktop에서는 248px 고정 Sidebar와 Topbar를 사용하고 1024px 미만에서는 Menu Button, Close, Backdrop, Escape와 Focus 복구를 지원하는 Drawer로 전환한다.
- 아직 구현되지 않은 화면은 Link가 아닌 `aria-disabled` 항목으로 표시해 존재하지 않는 기능으로 이동하지 않게 했다.
- 실제 소비자와 함께 검증해야 하는 Idempotency·Cursor·Run Owner·SSE는 WBS-05, Slack OIDC는 WBS-16, Reverse Proxy OIDC와 신뢰 Header 정책은 WBS-18로 소유권을 옮겼다.
- Browser 지원 범위는 현재 TypeScript/Vite Target과 표준 DOM·CSS 기능을 유지한다. 배포 Browser Matrix 확장은 WBS-19에서 Release Gate와 함께 결정한다.
...
- WBS-05: `api_idempotency_records`, Run 생성 Idempotency, Cursor, Run Owner와 Run Event SSE
- WBS-16: Slack OpenID Connect와 Slack Identity의 기존 User·Session 연결
- WBS-18: Reverse Proxy OIDC Header Adapter, Trusted Proxy와 최초 지원 Header 목록
- WBS-19: 배포 대상 Browser Matrix와 호환성 Release Gate
