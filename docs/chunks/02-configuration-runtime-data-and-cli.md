...
- [ ] `PangiConfig` Schema와 환경별 기본값을 정의한다.
- [ ] Config/Data/Log/Backup/Vault 경로 Resolver를 구현한다.
- [ ] `pangi init`의 Preview, 확인, 멱등 생성과 Non-interactive Mode를 구현한다.
- [ ] Project-local `.gitignore` Marker Block 갱신을 구현한다.
- [ ] `pangi start`, `status`, `version`, `config path/validate`를 연결한다.
- [ ] `doctor` 검사 Registry, 상태와 종료 코드를 구현한다.
- [ ] `doctor --offline`, `--json`, `--strict` 계약을 구현한다.
- [ ] Bootstrap URL을 일회성으로 생성하고 첫 Admin 생성 뒤 폐기하는 Port를 정의한다.

## 검증 체크리스트

- [ ] 깨끗한 임시 Home에서 `init`를 두 번 실행해 동일 결과를 확인한다.
- [ ] 기존 Config와 사용자 파일을 덮어쓰지 않는지 확인한다.
- [ ] Project-local Mode에서 `.pangi/`가 정확히 한 번 Ignore되는지 확인한다.
- [ ] `doctor --offline --json` Schema와 종료 코드 0/1/2를 Contract Test로 고정한다.
- [ ] Text/JSON 출력에 Secret Pattern이 나타나지 않는지 검사한다.
...
- 지원 OS별 기본 Config/Data 경로의 최종 명칭
- Non-interactive 설치 설정 파일의 배포 방식
