...
MCP_USER_AGENT = "Pangi-Git-MCP-Client/0.1"
MCP_PROTOCOL_VERSION = "2025-06-18"


class GitMcpError(RuntimeError):
    """Raised when the Git MCP server cannot return usable data."""


class GitMcpAuthError(GitMcpError):
    """Raised when Git MCP authentication fails."""
...
class GitMcpHttpClient:
    ...
    async def list_tools(self) -> tuple[GitMcpTool, ...]:
        result = await self._request("tools/list", {})
        raw_tools = result.get("tools") or []
        tools: list[GitMcpTool] = []
    ...
    async def _request(self, method: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        response, _headers = await asyncio.to_thread(self._post_json_rpc, payload, self._session_id)
    ...
    def _post_json_rpc(
        ...
        try:
            ...
            if error.code in {401, 403}:
                raise GitMcpAuthError("Git MCP authentication failed") from error
            raise GitMcpError("Git MCP HTTP request failed") from error
...
def _json_rpc_result(response: dict[str, Any]) -> dict[str, Any]:
    if "error" in response:
        raise GitMcpError("Git MCP returned JSON-RPC error")
    result = response.get("result")
    if not isinstance(result, dict):
        raise GitMcpError("Git MCP returned invalid JSON-RPC result")
    return result
...
def _parse_json_or_sse(raw: bytes) -> dict[str, Any]:
    ...
    if text.startswith("{"):
        ...
        if not isinstance(data, dict):
            raise GitMcpError("Git MCP returned invalid JSON")
        return data
    ...
    if not data_lines:
        raise GitMcpError("Git MCP returned unsupported response")
    data = json.loads("\n".join(data_lines))
    if not isinstance(data, dict):
        raise GitMcpError("Git MCP returned invalid SSE JSON")
    return data
